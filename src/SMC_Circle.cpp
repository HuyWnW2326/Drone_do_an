...
private:
    ...
    std::atomic<uint64_t> t_start;   //!< common synced timestamped
    std::atomic<uint64_t> t_end;   //!< common synced timestamped
    ...
    void Altitude_controller(float DesiredZ);
    void Controller_xy(float DesiredX, float DesiredY);
...
void OffboardControl::Altitude_controller(float DesiredZ)
{
    float k_z = 3.5f;
    float lamda_z = 2.0f;
    ...
    // float t_z = this->get_clock()->now().seconds();
    float delta_tz = this->get_clock()->now().seconds() - tz_last;
    float err_z = DesiredZ - pos_cur.z;

    if(delta_tz > 0)
    {
        vz_d = (DesiredZ - z_d_last) / delta_tz;
        az_d = (vz_d - vz_d_last) / delta_tz;
    }
    err_dot = vz_d - vel_cur.z;
    fz = (-m/(cosf(Roll_cur) * cosf(Pitch_cur))) * (az_d - g + lamda_z * err_dot + k_z * std::clamp(err_dot + lamda_z * err_z, -1.0f, 1.0f));
...
void OffboardControl::Controller_xy(float DesiredX, float DesiredY)
{
    float k_x = 3.0f;
    float lamda_x = 1.5f;
    ...
    float err_x = DesiredX - pos_cur.x;
    if(delta_tx > 0)
    {
        vx_d = (DesiredX - x_d_last) / delta_tx;
    ...
    float err_y = DesiredY - pos_cur.y;
    if(delta_ty > 0)
    {
        vy_d = (DesiredY - y_d_last) / delta_ty;

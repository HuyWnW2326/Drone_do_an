...
float OffboardControl::alt_controller(float desired_z)
{
    ...
    if(STEP_OFFBOARD == TAKEOFF)
        vel_z_d = saturation((desired_z - pos_cur.z) * kp2, -0.7f, 0.7f);
    else
        vel_z_d = saturation((desired_z - pos_cur.z) * kp2, -1.0f, 1.0f);

    float uz = kp1 * (vel_z_d - vel_cur.z);
    fz = ((g - uz) * m) / (cosf(roll_cur) * cosf(pitch_cur));
...
void OffboardControl::xyz_controller(float desired_x, float desired_y, float desired_z)
{
    ...
    std::array<float ,2> vel_xy = constrain_xy((desired_x - pos_cur.x) * kp2, (desired_y - pos_cur.y) * kp2, 2.0f);
